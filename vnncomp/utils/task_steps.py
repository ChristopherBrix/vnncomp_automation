...
class TaskStep(db.Model):
    ...
    def status_check(self):
        ...
        print(f"Status check {self._db_task} (currently in step {self})")
        if self._db_task.instance is None:
            print(f"No instance connected.")
            ...
            print(f"Instance {self._db_task.instance} connected.")
...
class ToolkitCreate(TaskStep):
    ...
    def _create(self):
        ...
        if self._db_task.eni is None:
            success = AwsManager.start_new_toolkit_instance(
                self._db_task.aws_instance_type, self._db_task.ami
            )
        else:
            success = AwsManager.start_new_toolkit_instance_with_eni(
                self._db_task.aws_instance_type, self._db_task.ami, self._db_task.eni
            )
        if success:
            db.session.commit()
            self._db_task.step_succeeded()
