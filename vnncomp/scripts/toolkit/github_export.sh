...
ssh -o StrictHostKeyChecking=accept-new -i ~/.ssh/vnncomp.pem ubuntu@${benchmark_ip} \
    "tmux new -d -s saving \" \
        bash -c \\\"
            ...
            cp logs/counterexamples/${benchmark_year}_${benchmark_name}/${benchmark_name}/* all_results/${tool_name}/${benchmark_year}_${benchmark_name}/
            cp logs/results_${benchmark_year}_${benchmark_name}.csv all_results/${tool_name}/${benchmark_year}_${benchmark_name}/results.csv \
                && cd all_results
            gzip ${tool_name}/${benchmark_year}_${benchmark_name}/*.counterexample
            ...
            git config --global user.name \\\\\\\"VNN-Comp Bot\\\\\\\" \
                ...
                && git add ${tool_name}/${benchmark_year}_${benchmark_name} \
                && git add ${tool_name}/results.csv \
                && git pull \
                && git status \
                && git commit -m \\\\\\\"Added results for ${tool_name} on ${benchmark_year}_${benchmark_name}\\\\\\\" \
                && git push \
        ...
        \\\" > >(tee logs/github_export_${benchmark_year}_${benchmark_name}.log) 2>&1 < /dev/null
    \" "